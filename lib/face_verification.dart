...
class AnnotatedImageIdentificationResult {
  /// Original image path that was processed.
  final String imagePath;

  /// Path to the generated image with green face boundaries.
  final String annotatedImagePath;

  /// Encoded bytes of the generated image with green face boundaries.
  final Uint8List annotatedImageBytes;

  /// Unique user IDs identified in this image.
  final List<String> userIds;

  /// Number of faces detected in the image.
  final int detectedFaceCount;

  AnnotatedImageIdentificationResult({
  ...
  @override
  String toString() {
    return 'AnnotatedImageIdentificationResult(imagePath: $imagePath, '
        'annotatedImagePath: $annotatedImagePath, '
        'detectedFaceCount: $detectedFaceCount, userIds: $userIds)';
  }
}
...
class _AnnotatedImageOutput {
  final String path;
  final Uint8List bytes;

  _AnnotatedImageOutput({required this.path, required this.bytes});
}
...
class FaceVerification {
  ...
  identifyAllUsersFromImagePathWithBoundingBoxes({
    ...
    if (lineThickness < 1) {
      throw ArgumentError('lineThickness must be >= 1');
    }
    ...
    if (sourceImageBytes == null) {
      throw Exception('Failed to read source image bytes');
    }
    ...
    return AnnotatedImageIdentificationResult(
      imagePath: imagePath,
      annotatedImagePath: annotatedOutput.path,
      annotatedImageBytes: annotatedOutput.bytes,
      userIds: computation.matchedUserIds.toList(),
      detectedFaceCount: computation.faces.length,
    );
  }

  Future<_ImageIdentificationComputation> _identifyUsersInImage({
    required String imagePath,
    required List<FaceRecord> candidateRecords,
    required double threshold,
    bool includeImageBytes = false,
  }) async {
    final inputImage = InputImage.fromFilePath(imagePath);
    final faces = await _detector.detectFaces(inputImage);
      ...
      try {
        ...
        for (final record in candidateRecords) {
          if (record.embedding.length == emb.length) {
            final score = cosineSimilarity(record.embedding, emb);
            if (score > bestScore) {
              bestScore = score;
              bestMatchId = record.id;
            }
  ...
  Future<_AnnotatedImageOutput> _generateAndPersistAnnotatedImage({
    ...
    await file.writeAsBytes(annotatedBytes, flush: true);
